...
export default function AdminOrdersPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-bold text-ink">{t('admin.orders')}</h2>
        <p className="text-xs text-ink-muted">{mockOrders.length} {t('common.results')}</p>
      </div>
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="border-b border-surface-muted text-left text-[11px] uppercase tracking-wide text-ink-muted">
                ...
                <th className="hidden px-4 py-2.5 font-semibold md:table-cell">{t('admin.date')}</th>
                <th className="hidden px-4 py-2.5 font-semibold sm:table-cell">{t('cart.items')}</th>
            ...
            <tbody className="divide-y divide-surface-muted">
              {list.map((o) => (
                <tr key={o.id} className="transition hover:bg-surface-soft">
                  ...
                  <td className="hidden px-4 py-2.5 text-ink-muted md:table-cell">{o.date}</td>
                  <td className="hidden px-4 py-2.5 text-ink-soft sm:table-cell">{o.items}</td>
                  <td className="px-4 py-2.5">
